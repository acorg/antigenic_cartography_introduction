---
title: "Antigenic Cartography Demonstration"
...
This page contains the code to make
antigenic maps and antibody landscapes.
...
variants may look very different. Still, they can both be "correct" as
they correctly represent the input data. To assess the suitability of
antigenic maps for the given data, map diagnostic steps are important.*
...
allowing the euclidean map distance to be larger than the titer target
distance without penalty. Here, the LOD was 16.
...
most serum groups. What we can also see is that P.1.1 (Gamma) titers are
often higher than the titers against the homologous variant, indicating
that the specific virus isolate (authentic virus neutralization assays)
has high reactivity. We can adjust for that by lowering it's reactivity
in the antigenic map below.
...
Characteristics of a single exposure map are that sera (open squares, coloured by homologous variant)
are located close to their infecting/homologous variant (filled circles) as they have the
...
whereas BA.1 has \<LOD in almost all sera).
...
2) previous + alpha sera (different homologous variant but similar antigenic
profile) 
...
On the left, we see the map from 3x Pfizer vaccine sera. On the right,
arrows point to the variants' positions in the 2x Pfizer map. The arrows
